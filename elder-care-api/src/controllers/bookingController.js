...
const bookingController = {
  ...
  createBookingByPackage: async (req, res) => {
    try {
      ...
      const job = await agenda.schedule(
        new Date(Date.now() + 60 *60* 1000), // 1 giờ sau
        "auto-cancel-booking",
      ...
      await agenda.every(
        "5 minutes",
        "emit-booking-if-unaccepted",
        {
          bookingId: newBooking._id.toString(),
          targetRole: targetRole,
        },
        {
          unique: { "data.bookingId": newBooking._id.toString() },
          // optional: specify timezone if necessary
        }
      );
  ...
  acceptBooking: async (req, res) => {
    ...
    try {
      ...
      booking.scheduleIds = booking.scheduleIds
        ? booking.scheduleIds.concat(scheduleIds)
        : scheduleIds;
      ...
      await agenda.cancel({ "data.bookingId": booking.toString() });
      await agenda.cancel({
        name: "emit-booking-if-unaccepted",
